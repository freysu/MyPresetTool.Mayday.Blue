...
class AnimationController {
  constructor(element, timerDisplay) {
    ...
    this.isPaused = false;
    this.colorInfoTemplate = document.querySelector('#colorInfo');
    this.colorRgbCache = {};

    // 初始化Canvas
    this.canvas = document.getElementById('glowstickCanvas');
    this.ctx = this.canvas.getContext('2d');
    // 检测Canvas支持
    if (!this.ctx) {
      // 浏览器不支持Canvas，提供回退方案
      const fallbackImage = document.createElement('img');
      document.getElementById('colorElement').appendChild(fallbackImage);
      return;
    }

    // 监听窗口大小变化事件
    window.addEventListener('resize', () => this.resizeCanvas());
    this.glowstickWidth = 102;
    this.glowstickHeight = 1000;
    this.lightHeight = this.glowstickHeight * 0.6;
    this.ringHeight = 50;
    this.buttonRadius = 30;
    this.grooveHeight = this.glowstickHeight * 0.4 - 100 - 60;
    this.grooveWidth = this.glowstickWidth * 0.8;
    this.silverRingHeight = 10;
  ...
  reset() {
    ...
    this.colorInfoTemplate.innerHTML = null;
  ...
  hexToRgb(hex) {
    if (this.colorRgbCache[hex]) {
      return this.colorRgbCache[hex];
    }
    ...
    const b = value & 255;
    const rgb = `rgb(${r}, ${g}, ${b})`;
    this.colorRgbCache[hex] = rgb; // 存储到缓存中
    return rgb;
  }

  updateColorInfo(colorName, hexColor, time) {
    this.colorInfoTemplate.innerHTML = `
  ...
  animate(currentTimeStamp = 0) {
    ...
    // 更新Canvas颜色
    this.updateCanvasColor(this.hexToRgb(hexColor));
  ...
  updateCanvasColor(hexColor) {
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    this.drawGlowArea(hexColor);
    this.drawMainBody();
    this.drawRing();
    this.drawButton();
    this.drawGroove();
  ...
  resizeCanvas() {
    ...
    // 根据新的大小重新计算各个部分的高度
    this.glowstickWidth = width;
    this.glowstickHeight = height;
    this.lightHeight = this.glowstickHeight * 0.6;
    this.ringHeight = 50;
    this.buttonRadius = 30;
    this.grooveHeight = this.glowstickHeight * 0.4 - 100 - 60;
  ...
  drawGlowArea(hexColor) {
    ...
    this.ctx.fillRect(
      (this.canvas.width - this.glowstickWidth) / 2,
      0,
      this.glowstickWidth,
      this.lightHeight,
    );
  ...
  drawMainBody() {
    const bodyGradient = this.ctx.createLinearGradient(
      ...
      this.lightHeight,
      0,
      this.glowstickHeight,
    );
    bodyGradient.addColorStop(0, '#ecece9');
    bodyGradient.addColorStop(1, '#e6e6e6');
    this.ctx.fillStyle = '#e2e3e3';
    this.ctx.fillRect(
      (this.canvas.width - this.glowstickWidth) / 2,
      this.lightHeight,
      this.glowstickWidth,
      this.glowstickHeight - this.lightHeight,
    );
  }

  drawRing() {
    const ringY = this.lightHeight;
    const ringGradient = this.ctx.createLinearGradient(0, ringY, 0, ringY + this.ringHeight);
    ...
    this.ctx.fillText(text, (this.canvas.width - textWidth) / 2, ringY + this.ringHeight / 2 + 6);
  }

  drawButton() {
    const buttonY = this.lightHeight + this.ringHeight + 45;
    ...
    this.ctx.beginPath();
    this.ctx.arc(this.canvas.width / 2, buttonY, this.buttonRadius, 0, Math.PI * 2);
    this.ctx.lineWidth = 10;
    this.ctx.strokeStyle = this.isGlowing ? '#00a4ff' : '#808080';
    this.ctx.stroke();
    this.ctx.fill();
    this.ctx.fillStyle = 'black';
    this.ctx.font = 'bold 30px Arial';
    const buttonText = 'M';
    const buttonTextWidth = this.ctx.measureText(buttonText).width;
    this.ctx.fillText(buttonText, (this.canvas.width - buttonTextWidth) / 2, buttonY + 10);
  }

  drawGroove() {
    const grooveY = this.glowstickHeight - this.grooveHeight - this.silverRingHeight * 2;
    const grooveGradient = this.ctx.createLinearGradient(
      0,
      grooveY,
      0,
      grooveY + this.grooveHeight,
    );
    grooveGradient.addColorStop(0, '#a6a6a6');
    grooveGradient.addColorStop(1, '#808080');
    this.ctx.fillStyle = '#d2d3d5';
    const grooveX = (this.canvas.width - this.grooveWidth) / 2;
    const halfGrooveWidth = this.grooveWidth / 2;
    ...
    this.ctx.lineTo(grooveX + this.grooveWidth - cornerRadius, grooveY);
    this.ctx.arc(
      grooveX + this.grooveWidth - cornerRadius,
      grooveY + cornerRadius,
      cornerRadius,
      Math.PI * 1.5,
      Math.PI * 2,
    );
    this.ctx.lineTo(grooveX + this.grooveWidth, grooveY + this.grooveHeight - cornerRadius);
    this.ctx.arc(
      grooveX + this.grooveWidth - cornerRadius,
      grooveY + this.grooveHeight - cornerRadius,
      cornerRadius,
      0,
      Math.PI * 0.5,
    );
    this.ctx.lineTo(grooveX + cornerRadius, grooveY + this.grooveHeight);
    this.ctx.arc(
      grooveX + cornerRadius,
      grooveY + this.grooveHeight - cornerRadius,
      cornerRadius,
    ...
    this.ctx.lineTo(grooveX, grooveY + cornerRadius);
    this.ctx.fill();
  }

  drawSilverRing() {
    const silverRingY = this.glowstickHeight - this.silverRingHeight;
    this.ctx.fillStyle = '#6e6f7f';
    this.ctx.fillRect(
      (this.canvas.width - this.glowstickWidth) / 2,
      silverRingY,
      this.glowstickWidth,
      this.silverRingHeight,
    );
  }
...
//     div.appendChild(document.createTextNode(`Name: ${colorName}`));
//     div.appendChild(document.createElement('br'));
//     div.appendChild(document.createTextNode(`Time: ${time}ms`));
//     div.appendChild(document.createElement('br'));
//     div.appendChild(document.createTextNode(`Hex: ${hexColor}`));
//     div.appendChild(document.createElement('br'));
//     div.appendChild(document.createTextNode(`RGB: ${this.hexToRgb(hexColor)}`));
//     fragment.appendChild(div);

//     // 缓存已有的子元素
//     if (this.element.firstChild) {
//       this.element.replaceChild(fragment, this.element.firstChild);
//     } else {
