1. 颜色主题配置界面要注重可视化呈现，比如用色卡形式展示 Base Color 下拉菜单中的颜色选项(只支持现有自定义格式的颜色代码,但是有对应映射的颜色 hex 值)，让用户能直观感受；对于动态颜色范围的配置，设计直观的滑块或输入框，方便用户调整并实时显示各范围占比总和。
2. 生成按钮要大一点! 进度条可以隐藏.
3. 手动输入预设代码文本的区域，大约有三四百行的预设代码内容,所以需要设置合适的输入框大小和格式提示，如在旁边显示示例格式，方便用户参照输入。

4. 区分非专业用户和高级用户, 提高网页的新用户有的可能是零相关经验没接触过的,又或者是有相关经验的但是没使用过这个网站的,又有的是想快速浏览网站,了解有什么功能再决定是否要操作

新用户引导
降低上手难度：清晰的弹窗指引能让新用户在初次接触 Mayday.Blue 预设工具时，不至于面对复杂功能无从下手。例如，首次使用时弹窗提示先上传音频文件，详细说明支持的格式及操作方法，能帮助用户顺利迈出第一步。
聚焦操作流程：每一个弹窗只专注于当前关键步骤，避免新用户被过多信息干扰。如在颜色主题配置环节，弹窗引导用户如何选择基础颜色以及合理分配动态颜色范围，使其注意力集中在该任务上，提高操作准确性。
增强用户信心：逐步引导的方式让用户能按部就班完成操作，每完成一步都有成就感，增加他们对使用该工具的信心，也提升了对工具的初始好感度。

可能造成干扰：过多的 prompt 弹窗可能会打断用户的操作思路，尤其是当用户已经对某个步骤有一定了解或者想要自主探索时，弹窗频繁弹出会显得繁琐。例如，一些有相关经验的新用户可能不需要详细的颜色主题配置引导，弹窗却依然强制出现。
影响操作效率：对于想要快速生成预设代码和预览效果的用户，弹窗引导的节奏可能过于缓慢。比如，熟练用户可能希望一次性完成音频和歌词上传、颜色配置等多个步骤，但弹窗会强制他们分步进行，浪费时间。

5. 使用设计软件创建清晰、易理解、美观的用户界面。对于 Mayday.Blue 预设工具网站，选择与五月天演唱会风格相符的色彩搭配，比如可以采用五月天经典的蓝色系作为主色调，搭配一些明亮的色彩作为点缀，营造出充满活力和热情的氛围。在界面布局上，要将各个功能模块划分清晰，比如将预设代码生成器和预设代码可视化工具分别放在不同的区域，并且用明显的标题或分割线隔开。文字说明要简洁明了，比如在上传文件的地方明确写出支持的格式，在颜色主题配置区域详细解释每个选项的含义，让用户能快速理解并操作。
