"use strict";
(self["webpackChunkmypresets_mayday_blue"] = self["webpackChunkmypresets_mayday_blue"] || []).push([["original_src_sentiment-zh_cn_web_min_js"],{

/***/ "./original_src/sentiment-zh_cn_web.min.js":
/*!*************************************************!*\
  !*** ./original_src/sentiment-zh_cn_web.min.js ***!
  \*************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var lodash_assign__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! lodash.assign */ "./node_modules/lodash.assign/index.js");
/* harmony import */ var lodash_assign__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(lodash_assign__WEBPACK_IMPORTED_MODULE_0__);
function defaultSetTimout() {
  throw new Error("setTimeout has not been defined");
}
function defaultClearTimeout() {
  throw new Error("clearTimeout has not been defined");
}
function runTimeout(e) {
  if (cachedSetTimeout === setTimeout) return setTimeout(e, 0);
  if ((cachedSetTimeout === defaultSetTimout || !cachedSetTimeout) && setTimeout) return cachedSetTimeout = setTimeout, setTimeout(e, 0);
  try {
    return cachedSetTimeout(e, 0);
  } catch (t) {
    try {
      return cachedSetTimeout.call(null, e, 0);
    } catch (t) {
      return cachedSetTimeout.call(this, e, 0);
    }
  }
}
function runClearTimeout(e) {
  if (cachedClearTimeout === clearTimeout) return clearTimeout(e);
  if ((cachedClearTimeout === defaultClearTimeout || !cachedClearTimeout) && clearTimeout) return cachedClearTimeout = clearTimeout, clearTimeout(e);
  try {
    return cachedClearTimeout(e);
  } catch (t) {
    try {
      return cachedClearTimeout.call(null, e);
    } catch (t) {
      return cachedClearTimeout.call(this, e);
    }
  }
}
function cleanUpNextTick() {
  draining && currentQueue && (draining = !1, currentQueue.length ? queue = currentQueue.concat(queue) : queueIndex = -1, queue.length && drainQueue());
}
function drainQueue() {
  if (!draining) {
    var e = runTimeout(cleanUpNextTick);
    draining = !0;
    for (var t = queue.length; t;) {
      for (currentQueue = queue, queue = []; ++queueIndex < t;) currentQueue && currentQueue[queueIndex].run();
      queueIndex = -1, t = queue.length;
    }
    currentQueue = null, draining = !1, runClearTimeout(e);
  }
}
function nextTick(e) {
  var t = new Array(arguments.length - 1);
  if (arguments.length > 1) for (var n = 1; n < arguments.length; n++) t[n - 1] = arguments[n];
  queue.push(new Item(e, t)), 1 !== queue.length || draining || runTimeout(drainQueue);
}
function Item(e, t) {
  this.fun = e, this.array = t;
}
function noop() {}
function binding(e) {
  throw new Error("process.binding is not supported");
}
function cwd() {
  return "/";
}
function chdir(e) {
  throw new Error("process.chdir is not supported");
}
function umask() {
  return 0;
}
function hrtime(e) {
  var t = .001 * performanceNow.call(performance),
    n = Math.floor(t),
    r = Math.floor(t % 1 * 1e9);
  return e && (n -= e[0], r -= e[1], r < 0 && (n--, r += 1e9)), [n, r];
}
function uptime() {
  var e = new Date(),
    t = e - startTime;
  return t / 1e3;
}
function getDefaultExportFromNamespaceIfNotNamed(e) {
  return e && Object.prototype.hasOwnProperty.call(e, "default") && 1 === Object.keys(e).length ? e["default"] : e;
}

var globalContext,
  cachedSetTimeout = defaultSetTimout,
  cachedClearTimeout = defaultClearTimeout;
globalContext = "undefined" != typeof window ? window : "undefined" != typeof self ? self : {}, "function" == typeof globalContext.setTimeout && (cachedSetTimeout = setTimeout), "function" == typeof globalContext.clearTimeout && (cachedClearTimeout = clearTimeout);
var currentQueue,
  queue = [],
  draining = !1,
  queueIndex = -1;
Item.prototype.run = function () {
  this.fun.apply(null, this.array);
};
var title = "browser",
  platform = "browser",
  browser = !0,
  argv = [],
  version = "",
  versions = {},
  release = {},
  config = {},
  on = noop,
  addListener = noop,
  once = noop,
  off = noop,
  removeListener = noop,
  removeAllListeners = noop,
  emit = noop,
  performance = globalContext.performance || {},
  performanceNow = performance.now || performance.mozNow || performance.msNow || performance.oNow || performance.webkitNow || function () {
    return new Date().getTime();
  },
  startTime = new Date(),
  process = {
    nextTick: nextTick,
    title: title,
    browser: browser,
    env: {
      NODE_ENV: "production"
    },
    argv: argv,
    version: version,
    versions: versions,
    on: on,
    addListener: addListener,
    once: once,
    off: off,
    removeListener: removeListener,
    removeAllListeners: removeAllListeners,
    emit: emit,
    binding: binding,
    cwd: cwd,
    chdir: chdir,
    umask: umask,
    hrtime: hrtime,
    platform: platform,
    release: release,
    config: config,
    uptime: uptime
  },
  require$$0 = {
    "一句话": -2,
    "一审宣判": -1,
    "一拖再拖": -1,
    "一文不值": -2,
    "一笑": 2,
    "三宝": 2,
    "上衣": 2,
    "下垂": -2,
    "不专业": -2,
    "不乖": -2,
    "不以为然": -2,
    "不便之处": -2,
    "不信": -1,
    "不信任": -3,
    "不健康": -2,
    "不公平": -2,
    "不可取": -2,
    "不可抗拒": 2,
    "不可爱": -2,
    "不可逆": -1,
    "不合理": -1,
    "不和谐": -2,
    "不喜欢": -2,
    "不好": -2,
    "不安": -1,
    "不安全": -2,
    "不完善": -2,
    "不尊重": -2,
    "不屑": -2,
    "不工作": -3,
    "不平等": -1,
    "不悦": -2,
    "不支持": -2,
    "不敏感": -2,
    "不敬": -2,
    "不方便": -2,
    "不明": -1,
    "不服气": -1,
    "不满": -2,
    "不满意": -2,
    "不知道": -2,
    "不确定": -1,
    "不确定性": -1,
    "不祥": 3,
    "不稳定": -2,
    "不耐烦": -2,
    "不能接受": -2,
    "不舒服": -2,
    "不要脸": -4,
    "不解": -2,
    "不诚实": -2,
    "不负责任": 2,
    "不足": -2,
    "不道德": -2,
    "不配": -2,
    "不错": 3,
    "不需要": -2,
    "不高兴": -2,
    "丑": -3,
    "丑闻": -3,
    "专用": 2,
    "丢失": -2,
    "丢弃": -1,
    "严厉": -2,
    "严惩": -3,
    "严格": 3,
    "严谨": 3,
    "严重": -2,
    "丧": -2,
    "丧偶": -1,
    "丧失抵押品赎回权": -2,
    "中伤": -2,
    "中断": -2,
    "中毒": -2,
    "主动": 1,
    "主管": 2,
    "举棋不定": -2,
    "乐观": 2,
    "乱": -2,
    "乳房": -2,
    "了不起": 4,
    "争议": -2,
    "争鸣": -1,
    "事": 1,
    "事故": -2,
    "事项": 1,
    "享受": 2,
    "享有": 2,
    "亮": 1,
    "亮度": 1,
    "亮点": 2,
    "亲切": 3,
    "亲情": 3,
    "亲爱": 2,
    "人渣": -4,
    "仇敌": -3,
    "从事": 1,
    "他妈的": -4,
    "付": -1,
    "代表作": 4,
    "代言": 2,
    "令人不安": -2,
    "令人印象深刻": 3,
    "令人叹为观止": 3,
    "令人失望": -2,
    "令人心寒": -1,
    "令人担忧": -3,
    "令人振奋": 3,
    "令人欣慰": 2,
    "令人沮丧": -2,
    "令人痛心": -2,
    "令人讨厌": -2,
    "令人难以忘怀": 1,
    "令人难以置信": -1,
    "令人震惊": -2,
    "优势": 2,
    "优点": 2,
    "优秀": 5,
    "优良": 3,
    "优雅": 2,
    "传染": -2,
    "传染性": -1,
    "伤亡": -2,
    "伤害": -2,
    "伤心": -2,
    "伤心欲绝": -3,
    "伤感": -2,
    "伤脑筋": -2,
    "伦理": 2,
    "伪科学": -3,
    "伪装": -1,
    "伪证": -3,
    "伪造": -3,
    "低估": -1,
    "低劣": -3,
    "低能儿": -3,
    "作假": -3,
    "你妈的": -4,
    "佩服": 3,
    "使...称叹": 3,
    "依赖": 2,
    "侮辱": -1,
    "侵权": -2,
    "侵略": -2,
    "促进": 1,
    "俏皮": 2,
    "保存": 2,
    "保护": 1,
    "保暖": 1,
    "保留": -1,
    "保证": 1,
    "信任": 1,
    "信心": 2,
    "信念": 1,
    "倒霉": -2,
    "倜傥": 2,
    "借口": -1,
    "倦怠": -2,
    "债务": -2,
    "值得": 2,
    "值得一": 2,
    "倾倒": -2,
    "倾注": 3,
    "假装": -1,
    "假货": -3,
    "偏向": -2,
    "偏见": -1,
    "停止": -1,
    "停滞": -2,
    "停车": -1,
    "健康": 2,
    "健忘": -2,
    "健身": 1,
    "偷": -2,
    "偷偷摸摸": -1,
    "傲慢": -2,
    "傻": -1,
    "傻瓜": -2,
    "像": 2,
    "僵局": -2,
    "允许": 1,
    "充足": 1,
    "先进": 1,
    "免疫": 1,
    "免费": 1,
    "免除": 2,
    "兑现": -2,
    "入狱": -2,
    "全神贯注": 1,
    "八卦": -2,
    "公平": 2,
    "公正": 2,
    "公鸡": -5,
    "关心": 2,
    "兴奋": 3,
    "兴趣": 1,
    "具有讽刺意味​​": -1,
    "具有里程碑意义": 2,
    "内疚": -3,
    "再次保证": 1,
    "冒犯": -2,
    "冒险": 2,
    "冠军": 4,
    "冤": -2,
    "冥想": 1,
    "冰雹": 2,
    "冲击": -2,
    "冲洗": -3,
    "冲突": -2,
    "冲突性": -2,
    "决定性": 1,
    "冷冷清清": -2,
    "冷漠": -2,
    "冷落": -2,
    "冷门": -2,
    "冷静": 2,
    "净度": 2,
    "凯旋": 4,
    "凶手": -2,
    "出轨": -2,
    "出逃": -1,
    "击碎": -1,
    "击退": -1,
    "分享": 1,
    "分享到": 1,
    "分心": -2,
    "分散": -2,
    "分散了": -2,
    "分离": -1,
    "切": -1,
    "切割": -1,
    "创意": 2,
    "创新": 1,
    "判决": -1,
    "利用": -2,
    "利益": 1,
    "制备": 1,
    "刺": -2,
    "刺伤": -2,
    "刺激": -3,
    "刺激性": -3,
    "刻板印象": null,
    "刻毒": -2,
    "削减": -2,
    "前景": 1,
    "前景堪忧": -3,
    "前锋": -2,
    "剧毒": -2,
    "功能强大": 2,
    "功能障碍": -2,
    "加强": 2,
    "加重": -2,
    "劣势": -2,
    "动机": 1,
    "助学金": 1,
    "励志": 2,
    "势不可挡": 2,
    "勇敢": 2,
    "勇气": 2,
    "勒死": -2,
    "勘探": 1,
    "勾结": -3,
    "包庇": -3,
    "千篇一律": -2,
    "千载难逢": 3,
    "升值": 2,
    "半信半疑": -2,
    "协议": 1,
    "卑鄙": -3,
    "卓越": 2,
    "单纯": -2,
    "占有欲": -2,
    "卡": -2,
    "印象": 3,
    "印象深刻": 3,
    "危害": -2,
    "危机": -3,
    "危言耸听": -2,
    "危险": -2,
    "厄运": -2,
    "压制": -1,
    "压力": -1,
    "压迫": -2,
    "厌倦了": -3,
    "厌恶": -3,
    "原谅": 1,
    "参加办法": 1,
    "友好": 2,
    "反应过度": -2,
    "反感": -3,
    "发人深省": -1,
    "发自内心": 3,
    "发誓": -2,
    "取消": -1,
    "取消比赛资格": -2,
    "受不了": -3,
    "受伤": -2,
    "受害": -3,
    "受害人": -3,
    "受害者": -3,
    "受理": 1,
    "受益": 2,
    "受限": -2,
    "受骗": -2,
    "叛逆": -3,
    "口交": -5,
    "句子": -2,
    "叫好": 2,
    "叫道": -2,
    "可以肯定": 1,
    "可信": 2,
    "可取": 2,
    "可怕": -2,
    "可怜": -2,
    "可恶": -3,
    "可悲是": -2,
    "可惜": -2,
    "可爱": 2,
    "可疑": -2,
    "可笑": -3,
    "可耻": -2,
    "可获免咎": 2,
    "可避免出现": -1,
    "合法": 1,
    "同情": -1,
    "同意": 1,
    "后卫": 1,
    "后悔": -2,
    "向往": -1,
    "吓了一跳": -2,
    "吓人": -2,
    "吓呆了": -2,
    "吓坏了": -2,
    "否认": -2,
    "含糊": -2,
    "吮吸": -3,
    "启发": 2,
    "启示": -2,
    "吸吮": -3,
    "吸引": 1,
    "吸引力": 2,
    "吸收": 1,
    "吹嘘": -2,
    "吹捧": -2,
    "呆滞": -2,
    "告诫": -2,
    "呛": -2,
    "呜呼": 3,
    "周到": 2,
    "呻吟": -2,
    "呼喊": -2,
    "咄咄逼人": -2,
    "咆哮": -3,
    "和平": 2,
    "和煦": 3,
    "哀悼": -2,
    "品味": 2,
    "哄骗": -2,
    "哇": 4,
    "哈": 2,
    "哈哈": 3,
    "哈哈哈": 3,
    "哑巴": -3,
    "哑弹": -2,
    "哗然": -2,
    "哭了": -1,
    "哭泣": -2,
    "哽咽": -2,
    "哽咽道": -2,
    "唉": -1,
    "商贩": -2,
    "善良": 3,
    "善解人意": 2,
    "喜剧": 1,
    "喜悦": 3,
    "喜欢": 2,
    "喧嚷": -1,
    "喧闹": -1,
    "嗜睡": -2,
    "嘲弄": -2,
    "嘲笑": -2,
    "囚犯": -2,
    "囚禁": -2,
    "回应": 2,
    "回避": -3,
    "团结": 1,
    "困": -2,
    "困境": -1,
    "困惑": -2,
    "困扰": -1,
    "困难": -2,
    "固体": 2,
    "固定": -1,
    "固执": -2,
    "在调解": 2,
    "地狱": -4,
    "坏蛋": -3,
    "坐立不安": -2,
    "坑蒙拐骗": -3,
    "块": -1,
    "坚不可摧": 2,
    "坚信": 1,
    "坚决": 2,
    "坚果": -3,
    "坦言": -1,
    "垂头丧气": -2,
    "垃圾邮件": -2,
    "垃圾邮件发送者": -3,
    "垄断": -2,
    "基本上": 1,
    "堆积": -1,
    "堕落": -2,
    "堵": -1,
    "堵嘴": -2,
    "增加": 1,
    "增益": 2,
    "士气低落": -2,
    "声讨": -2,
    "处于不利地位": -2,
    "处罚": -2,
    "复仇": -2,
    "复兴": 2,
    "复杂": 2,
    "复活": 2,
    "外伤": -3,
    "夜魔侠": 2,
    "大": 1,
    "大厅": -2,
    "大喜": 4,
    "大怒": -3,
    "大惊失色": -2,
    "大方": 2,
    "大胆": 2,
    "大赞": 3,
    "天上": 4,
    "天堂": 2,
    "天真": -2,
    "天赐": 4,
    "失业率": -2,
    "失去": -3,
    "失去人性": -2,
    "失望": -2,
    "失眠": -2,
    "失败": -2,
    "失速": -2,
    "头晕": -2,
    "夸大": -2,
    "夸张": -2,
    "奇": -2,
    "奇妙": 3,
    "奇怪是": -1,
    "奇迹": 3,
    "奋斗": -2,
    "奖励": 2,
    "奖牌": 3,
    "奖项": 3,
    "奴役": -2,
    "奸商": -2,
    "好": 2,
    "好处": 2,
    "好奇": 1,
    "好玩": 4,
    "好评": 3,
    "妈的": -4,
    "妒忌": -1,
    "妓女": -4,
    "委屈": -2,
    "委托": 2,
    "威胁": -2,
    "娘": -5,
    "娘们": -5,
    "娱乐": 2,
    "嫉妒": -2,
    "孔": -2,
    "孤独": -2,
    "孩子气": -2,
    "宁静": 2,
    "宇": 3,
    "安全": 1,
    "安慰": 2,
    "安抚": 1,
    "完善": 2,
    "完好": 2,
    "完成": 2,
    "完美": 3,
    "官司": -2,
    "宝": 2,
    "宝石": 1,
    "实战": -1,
    "实现": 1,
    "实用性": 2,
    "实质": 1,
    "审批": 2,
    "审查": -2,
    "审问": -2,
    "宣传": -2,
    "宣判": -2,
    "宣誓就职": -2,
    "害怕": -2,
    "害羞": -1,
    "宽容": 1,
    "宽松": 1,
    "宽限期": 1,
    "寂寞": -2,
    "富人": 2,
    "富有魅力": 3,
    "对": 1,
    "对号入座": -2,
    "对抗": -1,
    "射击": -2,
    "尊敬": 2,
    "小便": -4,
    "小姐": -2,
    "小心": -2,
    "小气": -2,
    "小白": -2,
    "小说": 2,
    "小霸王": -2,
    "尖刻": -3,
    "尖叫": -2,
    "尴尬": -2,
    "尸体": -1,
    "屁股": -4,
    "屄": -5,
    "屈辱": -3,
    "崇拜": 3,
    "崇敬": 2,
    "崩溃": -2,
    "巧": 2,
    "巨大": 3,
    "差": -2,
    "师爷": -2,
    "希望": 1,
    "帕利": -1,
    "帮助": 2,
    "平反": 2,
    "平庸": -3,
    "平淡": -2,
    "平静": 2,
    "幸存下来": 2,
    "幸存者": 2,
    "幸福": 3,
    "幸运": 3,
    "幸运是": 3,
    "幽默": 2,
    "庄严": -1,
    "庆幸": 4,
    "庆祝": 3,
    "应激": -2,
    "废墟": -2,
    "废话": -2,
    "延伸": 1,
    "延期": -1,
    "延迟": -1,
    "延长": 1,
    "开发": -2,
    "开导": 2,
    "开心": 3,
    "开朗": 2,
    "开脱": 2,
    "异化": -2,
    "异想天开": 1,
    "弊端": -3,
    "引人入胜": 3,
    "张力": -1,
    "弱": -2,
    "弱势": -2,
    "弱点": -2,
    "强": 2,
    "强人所难": -2,
    "强制": -1,
    "强制性": -1,
    "强化": 2,
    "强壮": 2,
    "强奸犯": -4,
    "强度": 2,
    "强盗": -2,
    "强调": -2,
    "归属": 1,
    "影响": -1,
    "征收": -1,
    "很简单": 1,
    "很酷东西": 3,
    "徒劳": 2,
    "得意": 2,
    "得意扬扬": 3,
    "得意洋洋": 3,
    "得罪": -2,
    "得罪了": -2,
    "徘": -2,
    "御史": -2,
    "微笑": 2,
    "心慌": -2,
    "心灰意冷": -2,
    "心生疑虑": -1,
    "心疼": -2,
    "心碎": -3,
    "心花怒放": 3,
    "忌": -1,
    "忐忑": -2,
    "忘记": -1,
    "忠实": 3,
    "忠诚": 3,
    "忠诚度": 3,
    "忧心忡忡": -2,
    "忧虑": -2,
    "忧郁": -1,
    "快乐": 3,
    "快乐可言": -2,
    "快捷": 2,
    "快活": 2,
    "忽略": -1,
    "忽视": -1,
    "怀疑": -1,
    "怀疑论者": -2,
    "怒气冲冲": -3,
    "怜悯": 2,
    "怠慢": -2,
    "急": -1,
    "急了": -2,
    "急切": 2,
    "性交": -4,
    "性感": 3,
    "怪": -1,
    "怪异": -2,
    "怯懦": -2,
    "恋情": 2,
    "恐吓": -2,
    "恐怖": -3,
    "恐惧": -2,
    "恐慌": -2,
    "恢复": 1,
    "恨": -3,
    "恭喜": 2,
    "恶人-2\r": null,
    "恶作剧": -1,
    "恶化": -3,
    "恶心": -3,
    "恶棍": -3,
    "恶毒": -2,
    "恶行": -1,
    "恼人": -2,
    "恼火": -2,
    "悍妇": -4,
    "悖论": -1,
    "悟": 2,
    "悲伤": -2,
    "悲切": -2,
    "悲剧": -2,
    "悲哀": -2,
    "悲怆": -2,
    "悲愤": -2,
    "悲痛": -2,
    "悲观": -2,
    "悼念": -2,
    "惆怅": -2,
    "惊": -2,
    "惊人": 3,
    "惊叹": 2,
    "惊呆了": -2,
    "惊奇": 2,
    "惊惶-2\r": null,
    "惊慌失措": -3,
    "惊艳": 4,
    "惊讶": 2,
    "惊险": 5,
    "惊魂": -2,
    "惜字": 2,
    "惨": -3,
    "惨不忍睹": -2,
    "惨淡": -2,
    "惨败": -3,
    "惩戒": -3,
    "惩罚": -2,
    "惭愧": -2,
    "想家": -2,
    "惹恼": -2,
    "惹恼了": -2,
    "愁眉苦脸": -1,
    "愉快": 3,
    "意义": -2,
    "意外": -2,
    "意愿": 2,
    "意气风发": 2,
    "愚蠢": -2,
    "感兴趣是": 2,
    "感叹": -2,
    "感染": -2,
    "感觉": 1,
    "感谢": 2,
    "愣神": -2,
    "愤世嫉俗者": -2,
    "愤怒": -2,
    "愤慨": -2,
    "愤懑": -2,
    "愿景": 1,
    "愿望": 1,
    "慈悲": 2,
    "憎恶": -3,
    "懊恼": -2,
    "懒得": -2,
    "懒惰": -1,
    "懦夫": -2,
    "成功": 2,
    "成名原因": 1,
    "成本": -2,
    "成熟": 2,
    "成长": 1,
    "战": -2,
    "战争": -2,
    "战斗": -1,
    "戡": -2,
    "戴": -1,
    "手无寸铁": -2,
    "打扰": -2,
    "打招呼": 1,
    "打断": -2,
    "打断了": -2,
    "打败了": -2,
    "扩展": 1,
    "扫地": -2,
    "扬言": -2,
    "扭曲": -2,
    "扰乱": -2,
    "批准": 2,
    "批评": -2,
    "扼流圈": -2,
    "承认": -1,
    "承诺": 1,
    "抑制": -1,
    "投机": -2,
    "抖动": -2,
    "抗议": -2,
    "抗议者": -2,
    "抗辩": -2,
    "折扣": -1,
    "折磨": -1,
    "抛弃": -2,
    "抢": -2,
    "抢劫": -2,
    "抢断": -2,
    "护理方法": 2,
    "报复": -2,
    "报料": -2,
    "报警": -2,
    "抱怨": -2,
    "抱怨道": -2,
    "抵制": -2,
    "抹杀": -2,
    "抽筋": -1,
    "抿着嘴": -2,
    "担心": -2,
    "拍摄": -1,
    "拍案而起": -3,
    "拒不": -2,
    "拒绝": -1,
    "拒绝了": -2,
    "拓展": 1,
    "拖": -1,
    "拘留": -2,
    "招呼": 1,
    "招徕": -2,
    "招标内容": 2,
    "拥抱": 1,
    "拮抗": -2,
    "拷问": -4,
    "拼命": -3,
    "持怀疑态度": -2,
    "挂羊头卖狗肉": -2,
    "指控": -2,
    "指责": -2,
    "挑刺": -5,
    "挑战": -1,
    "挑衅": -1,
    "挖苦": -2,
    "挣扎": -2,
    "挫败": -2,
    "振奋": 3,
    "挺举": -3,
    "损失": -3,
    "损害": -3,
    "授": 1,
    "掉落": -1,
    "掌声": 2,
    "排水": -2,
    "排除": -1,
    "探究": -2,
    "探索": 1,
    "接受": 1,
    "接合": 1,
    "接吻": 2,
    "推出": 1,
    "推广": 1,
    "推荐": 2,
    "推迟": -1,
    "提升": 1,
    "插科打诨": -2,
    "揭露": -1,
    "搞乱了": -2,
    "搞坏": -2,
    "搞砸了": -3,
    "搞笑": 4,
    "摇摆不定": -1,
    "摇晃": -2,
    "摊主": -2,
    "撒尿": -3,
    "撕裂": -2,
    "操作": -1,
    "操控": -1,
    "操纵": -1,
    "支持": 1,
    "支持者": 1,
    "收益": 2,
    "收藏": 2,
    "收费": -3,
    "收费标准": -2,
    "改善": 2,
    "改进": 2,
    "攻击": -1,
    "放弃": -1,
    "放心": 1,
    "放松": 2,
    "放缓": -1,
    "放逐": -1,
    "故障": -2,
    "敌人": -2,
    "敌意": -2,
    "敏锐": 1,
    "救市": -2,
    "救援": 2,
    "救生员": 4,
    "散播": -2,
    "敬佩": 3,
    "文盲": -2,
    "斗争": -2,
    "斜线": -2,
    "斧": -1,
    "新鲜": 1,
    "旗舰": 2,
    "无": -1,
    "无与伦比": 1,
    "无为": -2,
    "无力": -2,
    "无奈": -2,
    "无心": -2,
    "无忧无虑": 1,
    "无担保": -2,
    "无效": -2,
    "无敌": 2,
    "无牙": -2,
    "无用": -2,
    "无畏": 2,
    "无知": -2,
    "无精打采": -2,
    "无罪": 2,
    "无聊": -2,
    "无能": -2,
    "无行为能力": -2,
    "无视": -2,
    "无言以对": -2,
    "无赖": -2,
    "日照": 2,
    "旦": -2,
    "时髦": 2,
    "明快": 2,
    "明确": 1,
    "明确提出": 1,
    "昏睡": -2,
    "映入眼帘": 1,
    "是的": 1,
    "显着": 2,
    "显著": 1,
    "晕": -1,
    "景点": 2,
    "智能": 1,
    "智能化": 2,
    "暂停": -1,
    "暗杀": -3,
    "暴力": -3,
    "曝光": -1,
    "更大": 3,
    "更糟": -3,
    "替罪羊": -2,
    "最严厉": -2,
    "最低": -1,
    "最佳": 3,
    "最喜欢": 2,
    "最大": 3,
    "最强": 2,
    "最穷": -2,
    "最糟糕": -3,
    "最聪明": 2,
    "最脏": -2,
    "最黑暗": -2,
    "有影响": 2,
    "有效": 2,
    "有条不紊": 2,
    "有用": 2,
    "有罪": -3,
    "有趣": 4,
    "有远见": 3,
    "朗声道": -1,
    "期待已久": -1,
    "期望": 2,
    "未了": -2,
    "未定": -1,
    "未经批准": -2,
    "未经证实": -1,
    "机会": 2,
    "机遇": 2,
    "杀": -3,
    "杀敌": -3,
    "杀气": -3,
    "杂乱无章": -2,
    "杂役": -2,
    "权威": 1,
    "来头": 2,
    "来袭": -1,
    "松": -3,
    "松了口气": 2,
    "极权主义": -2,
    "枪": -1,
    "某种": 0,
    "查杀": -3,
    "柴": -3,
    "柴草": -3,
    "梦": 1,
    "梦幻": 4,
    "梦想": 1,
    "棘手": -2,
    "森森": -2,
    "模拟": -2,
    "模糊": -2,
    "欢乐": 2,
    "欢呼": 2,
    "欢喜": 3,
    "欢欣鼓舞": 4,
    "欢腾": 3,
    "欢迎": 2,
    "欣喜": 3,
    "欣喜若狂": 2,
    "欣快": 4,
    "欣赏": 2,
    "欲绝": -2,
    "欺凌": -2,
    "欺诈": -4,
    "欺负": -2,
    "欺骗": -2,
    "歇斯底里": -3,
    "正": 2,
    "正义": 2,
    "正确方向": 3,
    "正面": 2,
    "歹徒": -3,
    "死": -3,
    "死亡": -2,
    "死亡人数": -3,
    "死者家属": -2,
    "残酷": -3,
    "殴打": -2,
    "毁": -2,
    "毁灭": -2,
    "毁灭性": -2,
    "母犬": -5,
    "母狗": -5,
    "毒": -2,
    "毒药": -2,
    "比拟": 2,
    "毛": -2,
    "气势": -1,
    "气馁": -2,
    "汁液": -1,
    "求救": -2,
    "污垢": -2,
    "污染": -2,
    "污染者": -2,
    "沉思": -1,
    "沉着脸": -2,
    "沉船": -2,
    "沉闷": -2,
    "沉默": -1,
    "没头脑": -2,
    "没有乐趣": -3,
    "没有得到爱护": -2,
    "没用": -2,
    "没精打采": -2,
    "沮丧": -2,
    "油菜": -4,
    "泄漏": -1,
    "泄露": -1,
    "法律": 1,
    "法西斯": -2,
    "注定": -2,
    "泯": -2,
    "洗脑": -3,
    "洪琳": -2,
    "活力": 3,
    "活泼": 2,
    "活着": 1,
    "流感": -2,
    "流氓": -2,
    "流行": 3,
    "浩劫": -2,
    "浪费": -1,
    "浮力": 2,
    "涂片": -2,
    "消失": -1,
    "消极": -2,
    "消耗": -2,
    "淡然道": -2,
    "深情": 3,
    "混蛋": -4,
    "淹死": -2,
    "淹死了": -2,
    "清洁": 2,
    "清除": 1,
    "清零": 1,
    "温文尔雅": 2,
    "渴望": 2,
    "游": 3,
    "游说": -2,
    "滑头": 2,
    "滑稽": -2,
    "滞后": -1,
    "满目疮痍": -2,
    "满足": 2,
    "滥用": -3,
    "漂亮": 1,
    "漂绿": -3,
    "漏洞": -2,
    "演示": -1,
    "漠不关心": -2,
    "潇洒": 2,
    "潜伏": -1,
    "澄清": 2,
    "激动": -3,
    "激动不已": 5,
    "激励": 1,
    "激怒": -2,
    "激怒了": -2,
    "激情": 2,
    "激烈": 1,
    "激起了": -2,
    "灌输": -2,
    "火": -2,
    "火花": 1,
    "灰头土脸": -2,
    "灰色": -1,
    "灵巧": 2,
    "灾害": -2,
    "灾难": -2,
    "灾难性": -3,
    "炸弹": -1,
    "炽热": 2,
    "烁": 3,
    "烟": -2,
    "烟雾": -2,
    "烦": -2,
    "烦恼": -2,
    "烦躁": -2,
    "热切": 2,
    "热心": 1,
    "热情": 3,
    "热烈祝贺": 2,
    "热评": 2,
    "焦虑": -2,
    "煽动": -2,
    "煽动性": -2,
    "爆笑": 2,
    "爱": 3,
    "爱出风头": -1,
    "爱好": 2,
    "爱情": 3,
    "牵肠挂肚": -2,
    "特权": 2,
    "犯罪": -3,
    "犯罪嫌疑人": -1,
    "犹豫": -2,
    "狂": -3,
    "狂人": -3,
    "狂喜": 2,
    "狂暴": -3,
    "狂热": -3,
    "狂热者": -2,
    "狗屎": -4,
    "狠话": -1,
    "狡猾": -2,
    "独家": 2,
    "猥亵": -2,
    "猥琐": -2,
    "玩世不恭": -2,
    "珍惜": 2,
    "珍视": 2,
    "珠宝": 1,
    "理由": 2,
    "理直气壮": 2,
    "甜": 2,
    "生存": 2,
    "生气": -2,
    "生病": -2,
    "生闷气": -2,
    "疑虑": -2,
    "疑问": -1,
    "疯子": -3,
    "疯狂": -1,
    "疲劳": -2,
    "疲惫": -2,
    "疼痛": -1,
    "疾病": -2,
    "病": -2,
    "痒": -2,
    "痛苦": -2,
    "痛骂": -2,
    "痴情": 2,
    "痴迷": 2,
    "癌症": -1,
    "登上": 1,
    "白痴": -3,
    "百家争鸣": -1,
    "皮克": -2,
    "皮疹": -2,
    "皱眉道": -1,
    "监狱": -2,
    "盛大": 3,
    "盲人": -1,
    "盲目乐观": 2,
    "直": 1,
    "直索": -2,
    "相互矛盾": -2,
    "眉飞色舞": 3,
    "看点": 1,
    "看破红尘": -2,
    "真": 2,
    "真棒": 4,
    "真气": -2,
    "真诚": 2,
    "眼泪": -2,
    "着迷": 3,
    "矛盾": -1,
    "短视": -2,
    "破": -1,
    "破产": -3,
    "破坏": -2,
    "破坏力": -3,
    "硫酸": -3,
    "硬": -1,
    "确保": 1,
    "确定": 2,
    "碰撞": -1,
    "礼物": 2,
    "礼貌": 2,
    "祈祷": 1,
    "祝福": 1,
    "祝贺": 2,
    "神": 1,
    "神仙": 2,
    "神经": -1,
    "神话": -1,
    "神话般": 4,
    "祸患": -2,
    "禁止": -2,
    "禁用": -1,
    "离奇": -2,
    "离开": -1,
    "离线": -1,
    "种族主义": -3,
    "种族主义者": -3,
    "秘籍": -3,
    "称赞": 2,
    "移花接木": -3,
    "稳健": 2,
    "稳定": 2,
    "穆迪": -1,
    "穷": -2,
    "穷人": -2,
    "空": -1,
    "空虚": -1,
    "穿长袍": -2,
    "突出": 2,
    "突破": 3,
    "窒息": -1,
    "竞争性": 2,
    "竞争者": -1,
    "笑": 3,
    "笑声": 3,
    "笑话": 2,
    "笑道": 1,
    "笨蛋": -3,
    "笼罩": -1,
    "等待": -1,
    "简单化": -2,
    "类型": 2,
    "粉碎": -1,
    "粉饰": -3,
    "粗壮": 2,
    "粗心": -2,
    "粲": -3,
    "精力充沛": 2,
    "精彩": 3,
    "精益求精": 3,
    "精神": 1,
    "精美": 3,
    "紧张": -2,
    "紧张地问": -2,
    "紧急求助": -2,
    "累了": -2,
    "繁荣": 3,
    "红肿": -2,
    "织机": -1,
    "经济衰退": -2,
    "绑架": -2,
    "给予": 1,
    "绝望": -2,
    "统一": 1,
    "维护了": 2,
    "维生素": 1,
    "综合": 2,
    "绿洗": -3,
    "绿色洗涤": -3,
    "缓解": 1,
    "缩头缩脑": -2,
    "缺乏": -2,
    "缺乏自信": -2,
    "缺席者": -1,
    "缺点": -2,
    "缺陷": -2,
    "罚款": -1,
    "罢工": -1,
    "罪犯": -2,
    "美化": 3,
    "美味": 3,
    "美图": 3,
    "美眉": -1,
    "美艳": 3,
    "美观": 3,
    "美食": 3,
    "羞愧": -2,
    "羞辱": -2,
    "羡慕": -1,
    "翻牌": -2,
    "老张": -2,
    "耐力": 2,
    "耐寒": 2,
    "耗尽": -2,
    "耶": 1,
    "耶稣": 1,
    "耻辱": -2,
    "聚焦": -2,
    "聪明": 2,
    "肇事者": -2,
    "股份": 1,
    "育雏": -2,
    "肿瘤": -2,
    "胆怯": -2,
    "背叛": -3,
    "背负": -2,
    "胜": 4,
    "胡说": -2,
    "能力": 1,
    "脏": -2,
    "脱节": -2,
    "自信": 2,
    "自夸": -2,
    "自我迷惑": -2,
    "自杀": -2,
    "自满": -2,
    "自然": 1,
    "自由": 2,
    "自私": -3,
    "自责": -2,
    "臭名昭著": -2,
    "臭屁": -2,
    "致害": -3,
    "舀": 3,
    "舍弃": -1,
    "舒缓": 3,
    "舒适": 2,
    "舒适度": 2,
    "节日": 2,
    "芊芊": 4,
    "苛刻": -1,
    "若隐若现": -1,
    "苦": -2,
    "苦恼": -2,
    "苦笑": -2,
    "英勇": 3,
    "英雄": 2,
    "荣耀": 2,
    "荣获": 3,
    "荣誉": 2,
    "获奖": 4,
    "获得": 2,
    "获救": 2,
    "葬礼": -1,
    "蓄势": -2,
    "蔑视": -2,
    "藏": -1,
    "虚伪": -2,
    "虚弱": -2,
    "虚构": -1,
    "螺纹": -2,
    "蠢蛋": -4,
    "蠢驴": -4,
    "血腥": -3,
    "行为不端": -2,
    "行骗者": -4,
    "补助": 1,
    "表彰": 2,
    "表白": 2,
    "被动": -1,
    "被提": 2,
    "被盗": -2,
    "被砍": -1,
    "被骗": -2,
    "裹挟": -2,
    "要求": -1,
    "见死不救": -2,
    "规定": -1,
    "解决": 1,
    "解决办法": 1,
    "解决方案": 1,
    "解析": 2,
    "解疑": 2,
    "解禁": -2,
    "解表": 2,
    "触发器": -2,
    "警告": -2,
    "警惕": -1,
    "讨价还价": 2,
    "讨厌": -3,
    "讽刺": -1,
    "评论家": -2,
    "诈骗": -3,
    "诉讼": -1,
    "诙谐": 2,
    "诚信": 2,
    "诚实": 2,
    "该死": -4,
    "语重心长地说": 2,
    "误": -2,
    "误传": -2,
    "误区一": -2,
    "误导": -2,
    "误导性": -3,
    "误报": -2,
    "误解": -2,
    "误读": -1,
    "诱人": 1,
    "诱惑": -1,
    "说服": 1,
    "请": 1,
    "调解": 2,
    "谋杀": -2,
    "谎称": -2,
    "谨慎": -1,
    "谴责": -2,
    "豁免": -1,
    "豪言壮语人": -3,
    "负担": -2,
    "负责": 2,
    "负面": -2,
    "财富": 3,
    "责罚": -3,
    "贤惠": 2,
    "质疑": -1,
    "贪婪": -2,
    "贫困": -1,
    "贬低": -2,
    "贱人": -5,
    "贻害": -2,
    "贿赂": -3,
    "资产": 2,
    "赎回": 2,
    "赏识": -2,
    "赞同": 2,
    "赞成": 2,
    "赞扬": 2,
    "赞美": 3,
    "赞赏": 2,
    "赤字": -2,
    "赦免": 2,
    "走投无路": -2,
    "起诉": -1,
    "超强": 3,
    "超重": -1,
    "越狱": -1,
    "跆拳道": -4,
    "跛": -2,
    "跳动": -1,
    "跳跃": -1,
    "踏实": 2,
    "踩踏事故": -2,
    "转储": -1,
    "轻信": -2,
    "轻快": 2,
    "轻松": 1,
    "轻松活泼": 2,
    "轻率": -2,
    "轻蔑": -2,
    "轻视": -2,
    "辉煌": 2,
    "输家": -3,
    "辞职": -1,
    "辱骂": -3,
    "达到": 1,
    "迅速": 2,
    "过于简单化": -2,
    "过度反应": -2,
    "过度销售": -2,
    "过敏": -2,
    "过时": -2,
    "过载": -1,
    "运气": 3,
    "还原": 1,
    "进取": 1,
    "进展": 2,
    "远景": 1,
    "违反": -2,
    "违规": -2,
    "迪克": -4,
    "迫使": 1,
    "迫害": -2,
    "迷人": 3,
    "迷住": 3,
    "迷住了": 3,
    "迷失方向": -2,
    "迷惑": -2,
    "追问": -1,
    "退": -1,
    "退化": -2,
    "退让": -2,
    "适用于": 1,
    "逃脱": -1,
    "逃跑": -1,
    "逗": 3,
    "逗乐了": 3,
    "逮捕": -2,
    "道歉": -1,
    "遗忘": -1,
    "遗憾": -1,
    "避免": -1,
    "邀请": 1,
    "邪": -3,
    "郁闷": -2,
    "鄙视": -2,
    "配套": 1,
    "配料": -2,
    "酷": 1,
    "酷刑": -4,
    "醉": -2,
    "采用": 1,
    "释疑": -1,
    "重度": -2,
    "重磅炸弹": 3,
    "重罪": -3,
    "重要性": 2,
    "重要是": 2,
    "量刑": -2,
    "金德": 2,
    "钻机": -1,
    "钻石": 1,
    "铁青": -2,
    "错杀": -3,
    "错误": -2,
    "错过了": -2,
    "镦": -2,
    "闪避": -2,
    "闪闪发光": 3,
    "问候": 2,
    "问候语": 1,
    "问题": -2,
    "闷闷不乐": -2,
    "闹剧": -1,
    "闹鬼": -2,
    "防": -1,
    "防暴": -2,
    "阳痿": -2,
    "阴云密布": -1,
    "阴沉": -2,
    "阴谋": -3,
    "阴郁": -2,
    "阻力": -1,
    "阻挠": -2,
    "阻止": -1,
    "阻碍": -2,
    "阿飞": -2,
    "附魔": 2,
    "降": -1,
    "降低": -2,
    "限制": -1,
    "陷阱": -1,
    "隐藏": -1,
    "隔离": -1,
    "障碍": -2,
    "难点": -1,
    "雄心勃勃": 2,
    "雒": -3,
    "雨天": -1,
    "雨衣": 2,
    "需求": -1,
    "震惊": -2,
    "震撼": -2,
    "震耳欲聋": -1,
    "霍元甲": 2,
    "青春": 2,
    "青睐": 2,
    "非人": -2,
    "非人化": -2,
    "非人性化": -2,
    "非法": -3,
    "面色凝重": -2,
    "顶": 2,
    "预防": -1,
    "题为": 1,
    "颠覆性": -2,
    "颤抖": -2,
    "风扇": 3,
    "风景如画": 2,
    "风趣": 3,
    "风险": -2,
    "饥饿": -2,
    "饿死": -2,
    "驱逐": -1,
    "驴": -4,
    "骂": -2,
    "骂道": -1,
    "骄傲地说": 2,
    "骇然": -2,
    "验证": 1,
    "骗": -3,
    "骗子": -3,
    "骗局": -2,
    "骚乱": -2,
    "高估": -2,
    "高兴": 3,
    "高昂代价": 3,
    "高贵": 2,
    "高超": 5,
    "鬼": -1,
    "魅力": 3,
    "鲁莽": -2,
    "麻木": -1,
    "麻烦": -2,
    "黑人": -5,
    "黑客": -1,
    "黑暗": -1,
    "黑鬼": -5,
    "鼓励": 2,
    "鼓掌": 2,
    "鼓舞人心": 3
  },
  D_U_protection = ["妈的", "是的", "他妈的", "你妈的"],
  segmentit = window.Segmentit ? window.Segmentit.useDefault(new Segmentit.Segment()) : null;
if (!segmentit) throw new Error("Segmentit is not loaded");
var tokenize = function tokenize(e) {
    var t = [];
    segmentit.doSegment(e).forEach(function (e) {
      t.push(e.w);
    });
    var n = [];
    for (var r in t) {
      var o = t[r].replace(/[的得着了过]$/g, "");
      r >= 1 && -1 != D_U_protection.indexOf(t[r - 1] + t[r]) && (n.pop(), n.push(t[r - 1] + t[r])), -1 != D_U_protection.indexOf(t[r]) ? n.push(t[r]) : o && n.push(o);
    }
    return n;
  },
  afinn = require$$0,
  lib = function lib(e, t, n) {
    void 0 === e && (e = ""), void 0 === t && (t = null), "function" == typeof t && (n = t), void 0 === n && (n = null), null !== t && (afinn = lodash_assign__WEBPACK_IMPORTED_MODULE_0___default()(afinn, t));
    for (var r = tokenize(e), o = 0, u = [], i = [], a = [], c = r.length; c--;) {
      var l = r[c],
        m = afinn[l];
      afinn.hasOwnProperty(l) && (u.push(l), m > 0 && i.push(l), m < 0 && a.push(l), o += m);
    }
    var f = {
      score: o,
      comparative: o / r.length,
      tokens: r,
      words: u,
      positive: i,
      negative: a
    };
    if (null === n) return f;
    process.nextTick(function () {
      n(null, f);
    });
  };
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (lib);

/***/ })

}]);
//# sourceMappingURL=original_src_sentiment-zh_cn_web_min_js.bundle.js.map